$(function() {
    ...
    //     scrollGrowth('.main_visual article + article');
    //     scrollGrowth('.difference article');
    ...
    function setBannerSlide(selector) {
        ...
        $(selector).find('.indicator span').on('mousedown', function(e) {
            startX = e.clientX;
            offsetX = $(this).position().left;

            $(document).on('mousemove', function(e) {
                e.preventDefault();
                var dragBox = $(selector).find('.indicator').innerWidth();
                var dragBar = $(selector).find('.indicator span').innerWidth();
                var minOffsetDrag = 0;
                delX = e.clientX - startX;
                $(selector).find('.indicator span').css({'left': (offsetX + delX) + 'px'});
                minOffsetDrag = dragBox - dragBar;
                if ($(selector).find('.indicator span').position().left < minOffsetDrag) {
                    return false;
                }
            });
